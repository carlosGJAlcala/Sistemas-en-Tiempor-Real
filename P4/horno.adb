...
package body Horno is
   ...
   -- Retardo
   L: constant Float := 2.5;
   -- Periodo de la simulaci�n;
   Ts: constant := 0.1;
   package Retardador_de_Potencias is new
                   Retardadores (Retardo => L,
                               Elementos => Potencias,
                                  Tamanio => Positive(L/Ts));


   Ct: constant := 1_300.0; -- J/�C Capacidad t�rmica
   Cp: constant := 25.0; -- J/(�C*s) Coeficiente de p�rdidas
   T_Ambiente: constant Temperaturas := 25.0;
   ...
   -- Tarea que se encarga de cambiar peri�dicamente
   -- el estado del horno.
   task Centinela;
